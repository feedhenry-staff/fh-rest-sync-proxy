...
module.exports = function (dataset, opts) {
  ...
  log.debug(
    'creating MBaaS sync proxy for dataset "%s" with guid "%s" path "%s"',
    dataset,
    opts.guid,
    opts.pathToPrepend
  );
  ...
  function handlerCallback (name, callback) {
    return function (err, failureString, data) {
      if (err) {
        var e = new VError(
          err,
          'failed to perform "%s" for "%s" path "%s"',
          name,
          dataset,
          opts.pathToPrepend
        );
        ...
        log.debug(
          'sync call "%s" for "%s" was successful to "%s" path "%s"',
          name,
          dataset,
          opts.guid,
          opts.pathToPrepend
  ...
  ret.handleList = function handleList (dataset, params, callback) {
    ...
    http({
      ...
      method: 'GET',
      params: params,
      path: pathMod.normalize('/' + opts.pathToPrepend + '/' + dataset)
  ...
  ret.handleRead = function handleRead (dataset, id, callback) {
    ...
    http({
      ...
      guid: opts.guid,
      method: 'GET',
      path: pathMod.normalize('/' + opts.pathToPrepend + '/' + dataset + '/' + id.toString())
