...
var getHttpAdapter = require('./http')
...
module.exports = function (dataset, opts) {
  ...
  var ret = {};
  const http = getHttpAdapter(opts);
  ...
  function handlerCallback (name, callback) {
    return function (err, failureString, data) {
      console.log('handlerCallback for ',dataset,name);
      console.log(err);
      console.log(data);
      if (err) {
        var e = new VError(
   ...
   * @param  {Function} callback
   */
  ret.handleList = function handleList (dataset, params, metadata, callback) {
    console.log('handleList for ' + dataset);
    log.debug(
    ...
    var test = urlJoin(opts.url, buildRoute(opts, dataset));
    console.log(test);
    http({
      timeout: opts.timeout || DEFAULT_TIMEOUT,
      url: urlJoin(opts.url, buildRoute(opts, dataset)),
      method: 'GET',
      qs: params,
      json: true
   ...
   * @param  {Function} callback
   */
  ret.handleRead = function handleRead (dataset, id, metadata, callback) {
    log.debug(
    ...
    http({
      timeout: opts.timeout || DEFAULT_TIMEOUT,
      url: urlJoin(opts.url, buildRoute(opts, dataset), id.toString()),
      method: 'GET',
      json: true
   ...
   * @param  {Object}   md  metadata relating to the update
   * @param  {Function} callback
   */
  ret.handleUpdate = function handleUpdate (dataset, id, data, md, callback) {
    log.debug(
   ...
   * @param  {Object}   metadata  metadata relating to the delete
   * @param  {Function} callback
   */
  ret.handleDelete = function handleDelete (dataset, id, metadata, callback) {
    log.debug(
      'performing "%s" for dataset "%s" for id "%s"',
      'handleDelete',
      dataset,
   ...
   * @param  {Object}   metadata  metadata relating to the create
   * @param  {Function} callback
   */
  ret.handleCreate = function (dataset, data, metadata, callback) {
    log.debug(
      'performing "%s" for dataset "%s" with data %j',
      'handleCreate',
      dataset,
